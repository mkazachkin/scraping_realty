...
from market.utils.convertor import to_str, to_int, to_uuid, to_decimal, to_date, to_bool


class TOffersRooms (BaseModel):
    """
    Класс модели предложений комнат на сайтах продажи недвижимости
    """
    def __init__(self, is_commercial=False):
        self.is_commercial = is_commercial
        table_name = configuration.TABLES_CONFIG.get('rooms_characteristics')
        self._room_id = {
            'name': 'room_id',
            'convertor': to_uuid,
            'key_field': True,
    ...
    @property
    def floor_id(self):
        return self._floor_id.get('name')
    ...
    @property
    def quantity_rooms(self):
        return self._quantity_rooms.get('name')
    ...
    def has_furniture(self):
        return self._has_furniture.get('name')
